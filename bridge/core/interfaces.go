...
type Importer interface {
	...
	ImportAll(context.Context, *cache.RepoCache, time.Time) (<-chan ImportResult, error)
}

type Exporter interface {
	Init(conf Configuration) error
	ExportAll(context.Context, *cache.RepoCache, time.Time) (<-chan ExportResult, error)
}
